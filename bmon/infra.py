...
def _setup_bmon_common(user: str):
    fscm.s.pkgs_install('git supervisor docker.io')
...
def _mk_docker_run_executable(
        cmd_name: str, docker_run_args: str) -> fscm.PathHelper:
    """
    ...
    return (
        p(f"/usr/local/bin/bmon-{binname}", sudo=True)
        .contents(run_cmd)
        .chmod('755')
    )
...
def setup_grafana(port: int = 3000):
    ...
    _mk_docker_run_executable('grafana', (
        f'''
...
def setup_loki(
    ...
    _mk_docker_run_executable('loki', (
        f'''
        -p {port}:{port} --user=$(id -u) --network=host
        -v {datap}:/loki
        -v {etc}:/etc/loki
...
def setup_prometheus(
    ...
    _mk_docker_run_executable('prometheus', (
        f'''
...
def setup_alertmanager(
    ...
    _mk_docker_run_executable('alertmanager', (
        f'''
        {flags}
...
def setup_bitcoin_host(conf: MonitoredBitcoind, user: str):
    ...
    setup_bitcoind('', get_bitcoind_auth_line(conf.rpc_user, conf.rpc_password))
    setup_promtail(
        conf.loki_address, conf.version, '',
        str(BMON_DATA / "bitcoin" / "debug.log"),
    )
    setup_bitcoind_exporter(conf.rpc_user, conf.rpc_password)
    setup_prom_exporter()
...
def setup_promtail(
    ...
    _mk_docker_run_executable('promtail', (
        f'''
...
def _download_bitcoin(binary_dest_dir: Path):
    ...
    generally better.
    """
    if (bitcoind_bin := binary_dest_dir / 'bitcoind').exists() and \
            'v23.0.0' in run(f'{bitcoind_bin} -version | head -n 1', quiet=True).stdout:
        return
...
def setup_bitcoind_exporter(
    ...
    _mk_docker_run_executable('bitcoind_exporter', (
        f'''
...
def setup_prom_exporter():
    _mk_docker_run_executable('prom_exporter', (
        '''
